...
//import {searchPokemon} from "../../store/slices/selectedPokemonSlice";
...
export default function PokemonList() {
    ...
    useEffect(()=>{
        ...
        searchedPokemon.pokemon ?  setPokeList([searchedPokemon.pokemon]) : fetchPokemons() ;
    ...
    useEffect(()=>{
        async function fetchPokemonGenders(){
            ...
            const genderF = genderFemale.data.pokemon_species_details.map(item => item.pokemon_species.name)
            dispatch(setGenderList({female:genderF}))
            const genderMale = await axios("/gender/2");
            const genderM = genderMale.data.pokemon_species_details.map(item => item.pokemon_species.name)
            dispatch(setGenderList({male:genderM}))
            const genderUnKnown = await axios("/gender/3");
            const genderU = genderUnKnown.data.pokemon_species_details.map(item => item.pokemon_species.name)
