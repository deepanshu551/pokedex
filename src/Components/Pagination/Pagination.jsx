...
export default function Pagination() {
    ...
    const next = async ()=>{
        if(filteredList.pokemons){
            const newPokemons = filteredList.pokemons.slice(filteredList.nextSet,filteredList.nextSet+20);
    ...
    const prev=async()=>{
        if(filteredList.pokemons){
            const newPokemons = filteredList.pokemons.slice(filteredList.prevSet-20,filteredList.prevSet);
            dispatch(setPokemons({results:newPokemons}));
        ...
        const res = pokemon.pokemons.previous && await fetch(pokemon.pokemons.previous);
        if(res){
            const newPokemons = await res.json();
            dispatch(setPokemons(newPokemons));
            if(newPokemons.next){
  ...
  return (
    ...
    <button class="button previous" onClick={prev} disabled={disablePrev}>previous</button>
    <button class="button next" onClick={next} disabled={disableNext}>next</button>
</div>
  )
}
